...
def single_stranded_graph_with_simulated_depth(
    paths, depths, length=None, scale_depth_by=1
):
    if length is None:
    ...
    for p in paths:
        expected_depths[:, paths[p]] += np.outer(
            np.array(depths[p]) * scale_depth_by, np.ones(len(paths[p]))
        )
...
def ep_as_adjaceny(ep):
    return sp.sparse.csc_array(
        gt.spectral.adjacency(ep.get_graph(), weight=ep)
    ).toarray()
...
def draw_graph(g, output_size=(300, 300), ink_scale=0.8, **kwargs):
    kwargs = (
        dict(
            vertex_fill_color=g.new_vertex_property(
                "float", vals=np.linspace(0, 1, num=max(g.get_vertices()) + 1)
            ),
...
def dotplot(pathA, pathB, ax=None, **scatter_kws):
    ...
    pathA = np.pad(pathA, (0, length - len(pathA)), constant_values=-1)
    match = sp.spatial.distance.cdist(
        pathA.reshape((-1, 1)), pathB.reshape((-1, 1)), metric=lambda x, y: x == y
    )
...
def maximal_unitigs(g, include_singletons=False):
    no_sibling_edges = edge_has_no_siblings(g)
    g_no_sibling_edges = gt.GraphView(g, efilt=no_sibling_edges, directed=True)
    ...
    g_no_siblings_no_loops = gt.GraphView(
        g_no_sibling_edges, vfilt=isolated_loop_mask, directed=True
    )
    labels1, counts1 = gt.topology.label_components(
        g_no_siblings_no_loops, directed=False
    )
...
def mutate_add_compressed_unitig_vertex(
    g, vs, is_cycle, old_depths=None, drop_vs=False
):
    ...
    new_depth = (old_depths * g.vp.length.a[vs]).sum(1) / g.vp.length.a[v]
...
def mutate_compress_all_unitigs(g):
    ...
    for i, (vs, is_cycle) in enumerate(unitig_list):
        assert (
            len(vs) > 1
        )  # All len(vs) == 1 should be removed by include_singletons=False.
        mutate_add_compressed_unitig_vertex(
            g, vs, is_cycle, old_depths=old_depths[:, vs]
        )
        all_vs.extend(vs)
...
def mutate_extract_singletons(g):
    ...
    singletons = idxwhere(
        (nodes.in_degree + nodes.out_degree - 2 * nodes.circular) == 0
    )
...
def estimate_flow_old(f0, d, weight=None, eps=1e-2, maxiter=100):
    ...
    for step_i in range(maxiter):
        ...
        warn(
            f"loss_ratio < eps ({eps}) not achieved in maxiter ({maxiter}) steps. Final loss_ratio={loss_ratio}. Final loss={loss_hist[-1]}."
        )
    return f
...
def estimate_all_flows_old(g, eps=1e-3, maxiter=1000, use_weights=True, jobs=1):
    ...
    flows = map_f(
        _estimate_flow_old,
        [
            dict(f0=f0, d=dd[sample_idx], weight=weight, eps=eps, maxiter=maxiter)
...
def splits_from_sparse_encoding(g, v, threshold=1.0):
    # Compile tables
    ...
    out_neighbors_onehot = {
        k: v for k, v in zip(out_neighbors, np.eye(num_out_neighbors))
    }
    ...
    unnormalized_code = np.concatenate(
        [in_neighbor_code, np.ones((in_neighbor_code.shape[0], 1)), out_neighbor_code],
        axis=1,
    )
    ...
    for _ in range(code.shape[0]):
        ...
        normalized_encoding, _, _ = non_negative_factorization(
            obs,
...
def mutate_apply_splits(g, split_list):
    ...
    split_idx, upstream, downstream, lengths, depths = build_tables_from_splits(
        split_list, start_idx=start_idx
    )
    edges_to_add = list(
        set(
            new_edges_from_splits(
...
def splits_for_all_vertices(g, split_func, jobs=1):
    ...
    # code.
    non_linear_vs = [
        int(v) for v in g.vertices() if (v.in_degree() >= 2) or (v.out_degree() >= 2)
    ]
...
def mutate_run_workflow(g, thresh, maxiter=20):
    ...
    for i in range(maxiter):
        ...
        mutate_split_all_nodes(
            g, partial(splits_from_sparse_encoding, threshold=thresh)
        )
        num_edges, num_vertices = g.num_edges(), g.num_vertices()
        ...
        num_edges, num_vertices = g.num_edges(), g.num_vertices()
        info(
            f"Finished compressing unitigs (vertices: {num_vertices} edges: {num_edges})."
        )
        ...
        num_edges, num_vertices = g.num_edges(), g.num_vertices()
        info(
            f"Finished extracting singletons (vertices: {num_vertices} edges: {num_edges})."
        )
        if (num_vertices == 0) or (
    ...
    extracted_seqs = pd.concat(extracted_seqs + [last_extraction]).reset_index(
        drop=True
    )
